Shader "Custom/Dissolve_Voronoi_URP_Lit"
{
    Properties
    {
        // ─── Surface ─────────────────────────────────────────────────────
    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            // URP lighting keywords — must be multi_compile so the pipeline sets them
            ...
            struct Attributes
            {
                ...
                float4 tangentOS   : TANGENT;   // needed for TBN / normal map
                float2 uv          : TEXCOORD0;
            ...
            half4 FragmentFunction(Varyings IN) : SV_Target
            {
                ...
                half     shininess    = exp2(_Smoothness * 10.0 + 1.0); // remap 0-1 → usable range
                half3    specular     = _SpecColor.rgb * mainLight.color
                                       * mainLight.shadowAttenuation
                                       * pow(NdotH, shininess) * _Smoothness;

                // Ambient (URP spherical harmonics)
                half3    ambient      = SampleSH(normalWS) * albedo.rgb;

                // Additional lights (vertex or per-pixel based on URP asset setting)
                half3    addLights    = half3(0, 0, 0);
                    ...
                    // vertex lights baked into a TEXCOORD would need an extra interpolator;
                    // for Quest 3 we simply skip them here — the URP asset should be set
                    // to use vertex lights only if per-pixel count is > 0.
                #endif
        ...
        Pass
        {
            ...
            // _LightDirection is set by URP per-light during shadow rendering.
            // Declared explicitly for cross-platform compatibility (d3d11/Vulkan/Metal).
            ...
            ShadowVaryings ShadowVertexFunction(ShadowAttributes IN)
            {
                ...
                // URP's built-in helper applies shadow bias correctly
                float3 posWS       = TransformObjectToWorld(IN.positionOS.xyz);
                float3 normalWS    = TransformObjectToWorldNormal(IN.normalOS);
                float4 posCS       = TransformWorldToHClip(ApplyShadowBias(posWS, normalWS, _LightDirection));

                // Clamp depth to near plane to avoid shadow pancaking on Quest
                #if UNITY_REVERSED_Z
        ...
        // ── Depth Normals pass (SSAO / depth prepass on PCVR) ─────────────
        Pass
        {
            Name "DepthNormals"
            ...
            float4 DepthNormalsFragment(DNVaryings IN) : SV_Target
            {
                UNITY_SETUP_STEREO_EYE_INDEX_POST_VERTEX(IN);
                clip(VoronoiDistance(IN.positionOS) - saturate(_DissolveAmount));
                return float4(PackNormalOctRectEncode(
                    TransformWorldToViewDir(normalize(IN.normalWS), true)), 0, 0);
            }
